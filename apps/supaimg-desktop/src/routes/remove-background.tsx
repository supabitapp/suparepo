...
function RemoveBackgroundPage() {
  ...
  const config = getWorkflow("remove_bg");
  ...
  useEffect(() => {
    ...
    listenEvent<{ progress: number }>(EVENTS.modelDownloadProgress, (event) => {
      if (!active) return;
      ...
      .catch((err) => {
        void error(`model download listen failed: ${String(err)}`);
      });

    void invokeCommand(COMMANDS.prepareRemoveBgModels)
      .then(() => {
        if (!active) return;
        setProgress(1);
        setReady(true);
      ...
      .catch((err) => {
        ...
        void error(`model download failed: ${message}`);
  ...
  if (ready) {
    return <WorkflowPage workflow="remove_bg" />;
  }
  ...
  return (
    <WorkflowShell title={config.title}>
      <div className="flex flex-1 flex-col items-center justify-center gap-6">
        <ProgressView
          label={downloadError ? "Download failed" : "Downloading ML models for background removal"}
